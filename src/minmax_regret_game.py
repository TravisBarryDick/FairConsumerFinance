...
from regret_minimization_dp import weighted_regret_minimizing_products,regret_minimizing_products_jit
...
def minmax_regret_game(returns, groups, num_groups, num_prods, T,
                       use_avg_regret=True, step_size=None,use_it=False):
    """
    ...
    if use_it:
        for t in range(T):
            ...
            products[t, :] = regret_minimizing_products_jit(
                returns, num_prods, consumer_weights,)[1]
            #print(products)
            #print(products[t:,])
        ...
        for t in range(T):
            ...
            products[t, :] = weighted_regret_minimizing_products(
                returns, consumer_weights, num_prods,use_it)[1]
            #print(products)
            group_regrets[t, :] = get_group_regrets(returns, groups, num_groups,
                                                    products[t, :],
                                                    use_avg_regret=use_avg_regret)
