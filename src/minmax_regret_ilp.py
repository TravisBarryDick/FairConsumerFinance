...
def minmax_regret_ilp(users, num_prods, use_avg_regret=True, solver_str=None):
    """Solves an integer linear program to find the minmax regret-minimizing
       suite of products.

    Parameters
    ----------
    users : list or list-like
        A list of users' return thresholds.
    num_prods : int
        The number of products to choose.
    use_avg_regret : bool, optional
        If `True`, optimizes with respect to the average regret of a user in
        each group. If `False`, optimizes with respect to the total regret of
        all users in each group.
    solver_str : string, optional
        The name of a solver to pass to CVXPY.
    ...
    solvers = {'GUROBI': cp.GUROBI, "MOSEK": "MOSEK"}
